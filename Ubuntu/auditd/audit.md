...
이 문서는 **Ubuntu 서버에서 auditd + PLURA 에이전트**를 사용하여
...
* `ubuntu-auditd-attack-baseline.rules`  (기본/상시 권장 세트)
* `ubuntu-auditd-attack-deep.rules`      (심화·고볼륨 세트)
...
> ※ Ubuntu 기본값은 SELinux 대신 AppArmor이지만,
> SELinux를 활성화한 Ubuntu 환경에서도 RHEL과 유사하게 auditd 이벤트가 생성될 수 있습니다.
...
Ubuntu 에서는 `system-auth` 대신 **common-* 스택**을 사용합니다.
`/etc/pam.d/common-auth` 등에 `audit` 옵션을 포함할 수 있습니다.

```text
auth    [success=1 default=ignore] pam_unix.so nullok_secure audit
```

그리고 `sshd`, `login`, `sudo` 등 주요 서비스의 PAM 설정(예: `/etc/pam.d/sshd`)이
`@include common-auth` / `@include common-account` / `@include common-session` 등을 통해
해당 스택을 타도록 설정되어 있어야 합니다.

> **참고:**
> `ANOM_LOGIN_*` 류(로그인 실패 누적, 시간/위치 정책 위반 등)를 잘 활용하려면
> Ubuntu에서도 `pam_faillock` 또는 유사 정책(잠금 임계치 등)을 설정해야 합니다.

---

### 1-3. SELinux / AVC 관련 (SELinux 활성 Ubuntu 환경)

자동 생성되는 대표 이벤트 (SELinux 사용 시):
...
`/etc/selinux/config` 예 (SELinux를 사용하는 Ubuntu 환경):
...
아래는 대표적인 그룹입니다. (RHEL과 개념 동일, Ubuntu에 맞는 경로만 약간 다름)
...
* 서비스 유닛: `/etc/systemd/system/`, `/lib/systemd/system/`  ← **Ubuntu 경로**
* SELinux 정책: `/etc/selinux/…`, `/sys/fs/selinux/…` (SELinux 사용 시)
...
# systemd 서비스 유닛 변경 (Ubuntu: /lib/systemd/system/)
...
## 3. PLURA 전용 Ubuntu auditd 룰 세트

ATT&CK 관점에서 의미 있는 이벤트를 안정적으로 수집할 수 있도록,
PLURA는 Ubuntu용으로 아래 두 개의 규칙 파일을 제공합니다.

* **`ubuntu-auditd-attack-baseline.rules`**
  → 실서비스에 상시 적용 가능한 **기본 ATT&CK 세트**

* **`ubuntu-auditd-attack-deep.rules`**
...
### 3-1. ubuntu-auditd-attack-baseline.rules
...
# 32bit (멀티아치 시스템일 경우만 활성화, 필요시 주석 해제)
#-a always,exit -F arch=b32 -S execve \
#  -C auid>=1000 -F auid!=4294967295 \
#  -k plura_execve_user
...
# systemd 서비스 유닛 변경 (Ubuntu: /etc + /lib/systemd/system)
-w /etc/systemd/system/   -p wa -k plura_service_change
-w /lib/systemd/system/   -p wa -k plura_service_change
...
##########################################
# 6. SELinux / MAC 정책 변경 (SELinux 사용 Ubuntu 환경)
##########################################
...
### 3-2. ubuntu-auditd-attack-deep.rules
...
# 데이터 송수신 (간략 세트 – sendto/recvfrom 등)
-a always,exit -F arch=b64 \
...
# iptables/nft 실행 자체는 baseline 규칙(plura_fw_change)에서 이미 추적 중.
...
1. 위 두 파일을 Ubuntu 서버에 저장:

```bash
cp ubuntu-auditd-attack-baseline.rules /etc/audit/rules.d/
cp ubuntu-auditd-attack-deep.rules     /etc/audit/rules.d/   # (선택적)
```
...
* 모든 Ubuntu 서버:
  → `ubuntu-auditd-attack-baseline.rules` 만 먼저 적용
* 고위험/분석용 서버(예: 침해 의심 서버, PoC 환경):
  → `…-deep.rules` 를 추가 적용 (로그량·성능 영향 모니터링 필수)

---

필요하시면, **Ubuntu용 PAM 설정 예시(common-auth/common-session 전체 블록)** 이나
**SELinux 대신 AppArmor + auditd 연계 파트**도 따로 문단 파서 추가해 드릴게요.
