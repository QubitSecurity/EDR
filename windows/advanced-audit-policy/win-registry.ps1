<#
PLURA-Forensic
Windows Registry SACL(Audit ACL) applier for PLURA - Registry1 (This key only)
...
- RuleFileName을 지정하지 않으면 OS 역할에 맞는 rules 파일을 repo.plura.io에서 다운로드 후 적용
  * SERVER : https://repo.plura.io/edr/windows/advanced-audit-policy/server/s-audit-registry1.rules
  * DESKTOP: https://repo.plura.io/edr/windows/advanced-audit-policy/desktop/d-audit-registry1.rules
- RuleFileName이 URL(http/https)이면 다운로드 후 적용
- RuleFileName이 로컬 파일이면 로컬 파일 적용
- 로컬 파일이 없는데 파일명이 'd-audit-registry1.rules' 또는 's-audit-registry1.rules' 이면 해당 rules를 자동 다운로드 후 적용
- Proxy 지원:
  HKLM\SOFTWARE\QubitSecurity\PLURA\Proxy
- 출력은 Write-Output(stdout) + 파일 로그(C:\Program Files\PLURA\logs\win-registry.log)
...
Examples:
  ...
  # 로컬 rules 파일 지정(예)
  .\win-registry.ps1 .\d-audit-registry1.rules

  # URL 지정(예)
  .\win-registry.ps1 https://repo.plura.io/edr/windows/advanced-audit-policy/desktop/d-audit-registry1.rules

  # rules 목록만 출력
  .\win-registry.ps1 .\d-audit-registry1.rules List
#>
...
$DesktopRulesUrl = 'https://repo.plura.io/edr/windows/advanced-audit-policy/desktop/d-audit-registry1.rules'
$ServerRulesUrl  = 'https://repo.plura.io/edr/windows/advanced-audit-policy/server/s-audit-registry1.rules'
$DesktopRuleName = 'd-audit-registry1.rules'
$ServerRuleName  = 's-audit-registry1.rules'
$LogDir          = Join-Path $PluraRoot 'logs'
$LogFile         = Join-Path $LogDir 'win-registry.log'
...
function Apply-RegistryAuditRule_KeyOnly {
    param(
    ...
    if (-not (Is-RegistryPath -Path $t)) {
        throw "win-registry.ps1는 레지스트리 전용입니다: $t"
    }
...
try {
    ...
    if ($isSingleMode) {
        Write-Log -Level 'INFO' -Message ("Applying Registry auditing (SACL) - This key only (single target)")
        ...
        Apply-RegistryAuditRule_KeyOnly -TargetPath $Path -Perm $Perm
    ...
    Write-Log -Level 'INFO' -Message ("Applying Registry auditing (SACL) - This key only")
    ...
    foreach ($it in $items) {
        ...
        try {
            Apply-RegistryAuditRule_KeyOnly -TargetPath $t -Perm $it.Perm
            $appliedCount++
        }
        catch {
