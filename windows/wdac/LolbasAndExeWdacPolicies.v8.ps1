...
function New-DeterministicGuid {
    param(
        ...
        [string]$NamespaceGuid = '6ba7b811-9dad-11d1-80b4-00c04fd430c8'
    )

    $ns = [Guid]$NamespaceGuid
    $nameBytes = [System.Text.Encoding]::UTF8.GetBytes($Name)
    $nsBytes = $ns.ToByteArray()

    # Convert namespace GUID to RFC 4122 network byte order for UUIDv5 hashing.
    $nsRfc = New-Object byte[] 16
    $nsRfc[0] = $nsBytes[3]; $nsRfc[1] = $nsBytes[2]; $nsRfc[2] = $nsBytes[1]; $nsRfc[3] = $nsBytes[0]
    $nsRfc[4] = $nsBytes[5]; $nsRfc[5] = $nsBytes[4]
    $nsRfc[6] = $nsBytes[7]; $nsRfc[7] = $nsBytes[6]
    for ($i = 8; $i -lt 16; $i++) { $nsRfc[$i] = $nsBytes[$i] }

    $buffer = New-Object byte[] ($nsRfc.Length + $nameBytes.Length)
    [Array]::Copy($nsRfc, 0, $buffer, 0, $nsRfc.Length)
    [Array]::Copy($nameBytes, 0, $buffer, $nsRfc.Length, $nameBytes.Length)
    ...
    # UUID version 5 and RFC 4122 variant.
    $uuid[6] = ($uuid[6] -band 0x0F) -bor 0x50
    $uuid[8] = ($uuid[8] -band 0x3F) -bor 0x80

    # Convert back from RFC 4122 network byte order to .NET Guid byte order.
    $guidBytes = New-Object byte[] 16
    $guidBytes[0] = $uuid[3]; $guidBytes[1] = $uuid[2]; $guidBytes[2] = $uuid[1]; $guidBytes[3] = $uuid[0]
    $guidBytes[4] = $uuid[5]; $guidBytes[5] = $uuid[4]
    $guidBytes[6] = $uuid[7]; $guidBytes[7] = $uuid[6]
    for ($i = 8; $i -lt 16; $i++) { $guidBytes[$i] = $uuid[$i] }

    return ([Guid]::new($guidBytes)).ToString('B').ToUpperInvariant()
}

function Set-DeterministicPolicyIdInXml {
    param(
        [Parameter(Mandatory)]
        [string]$XmlPath,
    ...
    if ($null -eq $xml.SiPolicy) {
        throw "SiPolicy root not found in XML: $XmlPath"
    }

    if ($null -eq $xml.SiPolicy.PolicyID) {
        $node = $xml.CreateElement('PolicyID', $xml.SiPolicy.NamespaceURI)
        [void]$xml.SiPolicy.AppendChild($node)
    }
    if ($null -eq $xml.SiPolicy.BasePolicyID) {
        $node = $xml.CreateElement('BasePolicyID', $xml.SiPolicy.NamespaceURI)
        if ($xml.SiPolicy.PolicyID) {
            [void]$xml.SiPolicy.InsertBefore($node, $xml.SiPolicy.PolicyID)
        }
        else {
            [void]$xml.SiPolicy.AppendChild($node)
        }
    ...
    $xml.SiPolicy.PolicyID = $PolicyId
    $xml.SiPolicy.BasePolicyID = $PolicyId
    $xml.Save($XmlPath)
}
    ...
    Get-ChildItem -LiteralPath $OutputPath -Force -ErrorAction SilentlyContinue |
        Where-Object {
            ...
            (-not $_.PSIsContainer -and (
                $_.Name -like "$Prefix*.xml" -or
                $_.Name -like "$Prefix*.cip" -or
                $_.Name -eq $ManifestFileName
            ))
        } |
...
for ($index = 0; $index -lt $entries.Count; $index++) {
    ...
    try {
        ...
        if ($PSCmdlet.ShouldProcess($displayName, 'Generate WDAC deny XML/CIP')) {
            ...
            $policyId = New-DeterministicGuid -Name $policyName
            Set-DeterministicPolicyIdInXml -XmlPath $xmlPath -PolicyId $policyId
            ...
            $rootManifestItems.Add([PSCustomObject][ordered]@{
                ...
                PolicyId = $policyId
                XmlFileName = $xmlName
                BinaryFileName = $(if ($SkipCipConversion) { $null } else { $cipName })
                DeploymentBinaryFileName = $(if ($CreateDeploymentCopies -and -not $SkipCipConversion) { $deployCipName } else { $null })
                Targets = @($fileName)
                ...
                RelativeXmlPath = $xmlName
                RelativeBinaryPath = $(if ($SkipCipConversion) { $null } else { $cipName })
