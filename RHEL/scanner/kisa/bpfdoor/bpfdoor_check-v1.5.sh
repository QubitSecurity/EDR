#!/usr/bin/env bash
# bpfdoor_check.sh - BPFDoor integrated triage (YARA-free) for PLURA-Forensic
#
# VERSION  : 1.5
# REVISION : 2025-12-25-r6
# SCHEMA   : bpfdoor_check_plura_v1
#
# v1.5 goal:
#   - No external allowlist required.
#   - "Baseline absorption" is done automatically for known-benign system binaries
#     when ONLY weak indicators are present.
#
...
#   5) (Optional) strings triage on executable of FOUND PIDs only (KISA section 8)
...
#        C) RPM ownership/verification for the executable (RHEL/CentOS)
...
#        - Requires: magic_in_pid=0 AND rpm_verify=clean (when rpm is available)
#        - baseline=1 -> severity_final=LOW and counted as non-actionable
#        - Exit code becomes 0 if ONLY baseline findings exist (to be automation-friendly)
#
# Exit codes:
...
#
# Optional external allowlist (still supported, but not required):
#   /etc/bpfdoor_check.allow or ./bpfdoor_check.allow
#   Format: one rule per line, comma-separated key=glob tokens
#   Supported keys: comm, exe_real, exe, rpm_pkg
#
# Notes:
...
# Optional allowlist file (not required)
DO_ALLOWLIST=1
...
kv_add() {
  ...
  if [[ -z "$v" ]]; then
    ...
    # normalize newlines to escaped sequences
    v="${v//$'\r'/\\r}"
    v="${v//$'\n'/\\n}"
    v="${v//\\/\\\\}"
    v="${v//\"/\\\"}"
    ...
    # quote values containing spaces/tabs/quotes
    v="${v//\\/\\\\}"
    v="${v//\"/\\\"}"
    enc="\"$v\""
  else
...
oneline_trunc() {
  local s="$1"
  local max="${2:-900}"
  # collapse CR/LF to '; ' and squeeze whitespace
...
declare -A V_RPM_PKG          # pid -> package / unowned / N/A
declare -A V_RPM_VERIFY       # pid -> clean/changed/unowned/not_available/error
declare -A V_RPM_VERIFY_OUT   # pid -> string

# baseline storage (per PID)
declare -A V_BASELINE         # pid -> 0/1
declare -A V_BASELINE_SOURCE  # pid -> auto|builtin|allowlist|none
declare -A V_BASELINE_MATCH   # pid -> rule/reason string
...
resolve_allowlist_file() {
  # choose allowlist file path (explicit > default > fallback)
  if [[ -n "$ALLOWLIST_FILE" ]]; then
    printf '%s' "$ALLOWLIST_FILE"; return
  fi
  if [[ -r "$ALLOWLIST_FILE_DEFAULT" ]]; then
    printf '%s' "$ALLOWLIST_FILE_DEFAULT"; return
  fi
  if [[ -r "$ALLOWLIST_FILE_FALLBACK" ]]; then
    printf '%s' "$ALLOWLIST_FILE_FALLBACK"; return
  fi
  printf '%s' "" # none
}
...
# match a single rule line against values
# rule: token list separated by ',' where each token is key=glob
# supported keys: comm, exe_real, exe, rpm_pkg
rule_matches() {
  local rule="$1"
  local comm="$2"
  ...
  for tok in "${tokens[@]}"; do
    ...
    # glob match (pattern is unquoted on purpose)
    if [[ "$val" != $pat ]]; then
      return 1
    fi
  done
...
baseline_eligible_weakonly() {
  # baseline only for "weak-only" findings (no strong indicators)
  local reasons="$1"
  [[ -z "$reasons" ]] && return 1
  local r=",$reasons,"
  ...
  if [[ "$r" == *",RPM_VERIFY_CHANGED,"* ]]; then return 1; fi
  return 0
}

# Built-in baseline rules (NO external file needed)
# Add rules conservatively: only for well-known system daemons with rpm_verify=clean.
BASELINE_BUILTIN_RULES=(
  'comm=NetworkManager,exe_real=/usr/sbin/NetworkManager,rpm_pkg=NetworkManager-*'
)
BUILTIN_RULES_COUNT="${#BASELINE_BUILTIN_RULES[@]}"

# Load optional allowlist once at startup
load_allowlist
...
# KISA "3-3" uses hex values converted to decimal; keep for explicit parity
...
if [[ "$DO_VERIFY" -eq 1 && "${#PIDS_SORTED[@]}" -gt 0 ]]; then
  for pid in "${PIDS_SORTED[@]}"; do
    ...
    if [[ -n "$SS_OUT" ]]; then
      # precise pid match: pid=<PID> followed by non-digit or EOL (usually comma)
      ...
      # A) magic sequences presence within this PID's BPF evidence lines
    ...
    # C) RPM ownership + verify (RHEL/CentOS)
    exe_real="$(proc_exe_real "$pid")"
    if ! have rpm; then
      V_RPM_VERIFY["$pid"]="not_available"
      V_RPM_PKG["$pid"]="N/A"
      V_RPM_VERIFY_OUT["$pid"]=""
    elif [[ -z "$exe_real" || ! -e "$exe_real" ]]; then
      V_RPM_VERIFY["$pid"]="error"
      V_RPM_PKG["$pid"]="N/A"
      V_RPM_VERIFY_OUT["$pid"]="exe_real_missing"
    else
      rpm_pkg="$(rpm -qf "$exe_real" 2>/dev/null || true)"
      if [[ -z "$rpm_pkg" ]]; then
        V_RPM_VERIFY["$pid"]="error"
        V_RPM_PKG["$pid"]="N/A"
        V_RPM_VERIFY_OUT["$pid"]="rpm_qf_failed"
      elif [[ "$rpm_pkg" == *"is not owned"* ]]; then
        V_RPM_VERIFY["$pid"]="unowned"
        V_RPM_PKG["$pid"]="unowned"
        V_RPM_VERIFY_OUT["$pid"]=""
      else
        V_RPM_PKG["$pid"]="$rpm_pkg"
        # Verify only lines relevant to this executable to reduce noise
        ver_out="$(rpm -V "$rpm_pkg" 2>/dev/null | grep -F "$exe_real" || true)"
        if [[ -z "$ver_out" ]]; then
          V_RPM_VERIFY["$pid"]="clean"
          V_RPM_VERIFY_OUT["$pid"]=""
        else
          V_RPM_VERIFY["$pid"]="changed"
          V_RPM_VERIFY_OUT["$pid"]="$(oneline_trunc "$ver_out" 900)"
          # Raise forensic confidence (doesn't imply BPFDoor, but indicates tampering)
          add_reason "$pid" "RPM_VERIFY_CHANGED" 10
        fi
      fi
    fi
  done
...
BASELINE_AUTO_COUNT="0"  # reserved for future broad heuristics (currently 0)
...
if [[ "${#PIDS_SORTED[@]}" -gt 0 ]]; then
  for pid in "${PIDS_SORTED[@]}"; do
    ...
    # Extra safety gates for auto/builtin baseline:
    #  - no magic in this PID lines
    #  - rpm verification must be clean (if rpm is available; on non-rpm systems, do not auto baseline)
    if [[ "${V_MAGIC_IN_PID[$pid]:-0}" != "0" ]]; then
      continue
    fi
    if [[ "${V_RPM_VERIFY[$pid]:-not_available}" != "clean" ]]; then
      continue
    fi
    ...
    exe_link="$(proc_exe_link "$pid")"
    exe_real="$(proc_exe_real "$pid")"
    rpm_pkg="${V_RPM_PKG[$pid]:-}"

    # 5-1) external allowlist match (optional)
    if [[ "$ALLOWLIST_LOADED" -gt 0 ]]; then
      for rule in "${ALLOW_RULES[@]}"; do
        if rule_matches "$rule" "$comm" "$exe_real" "$exe_link" "$rpm_pkg"; then
          V_BASELINE["$pid"]="1"
    ...
    # 5-2) built-in baseline match (NO file required)
...
if [[ "$FORMAT" == "text" ]]; then
  ...
  echo "[VERIFY] $VERIFY_STATUS (A/B/C for every finding PID)"
  ...
  for pid in "${PIDS_SORTED[@]}"; do
    ...
    # optionally suppress baseline from output
    ...
    if [[ "$DO_VERIFY" -eq 1 ]]; then
      echo "verify  : bpf_line_count=${V_BPF_LINE_COUNT[$pid]:-0} magic_in_pid=${V_MAGIC_IN_PID[$pid]:-0} rpm_verify=${V_RPM_VERIFY[$pid]:-N/A} rpm_pkg=${V_RPM_PKG[$pid]:-N/A}"
      if [[ -n "${V_BPF_LINES_SAMPLE[$pid]:-}" ]]; then
        echo "bpf_ss  : ${V_BPF_LINES_SAMPLE[$pid]}"
      fi
      if [[ -n "${V_RPM_VERIFY_OUT[$pid]:-}" ]]; then
        echo "rpm_v   : ${V_RPM_VERIFY_OUT[$pid]}"
      fi
    fi
  ...
  # Result line (actionable vs baseline)
  ...
  # exit code depends on whether any actionable finding exists
